...
def main():
    ...
    if args.use_wandb:
        args.project_name = args.dataset + '_original'
        wandb.init(config=args, project=args.project_name, entity='lxc')
    ...
    tuning_all_after_iters = 40000//args.batch_sizes[0]
    tuning_all = False
    ...
    def val_step(val_loader, max_iter=5, log=True):
        VAE.eval()
    ...
    def generate(test_loader, num_iters, max_iter=5, log=True):
        VAE.eval()
        ...
        if args.use_wandb and log:
            wandb.log({'bleu4': bleu4*100, 'rouge1': rouge_scores_values[2], 'rouge2': rouge_scores_values[5],
                       'rouge3': rouge_scores_values[8]})

        VAE.train()

    # test_plot(test_loader, num_iters)
    val_step(val_loader, 5, False)
    generate(test_loader, num_iters, max_iter=5, log=False)
    ...
    while num_iters < args.iterations:
        ...
        with tqdm(total=len(train_loader)) as pbar:
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask) in enumerate(train_loader):
                ...
                if num_iters % 5000 == 0:
                    # test_plot(test_loader, num_iters)
                    val_step(val_loader, max_iter=len(val_loader), log=True)
                    generate(test_loader, num_iters, max_iter=len(test_loader), log=True)

                if num_iters % 5000000 == 0:
                    print('Saving model...')
